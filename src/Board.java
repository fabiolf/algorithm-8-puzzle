...
public class Board {
	...
	public Board twin() {
		...
		// uniformly random select any tile except for the blank
		int dimension = dimension();
		int iRandom, jRandom;
		do {
			iRandom = StdRandom.uniform(dimension);
			jRandom = StdRandom.uniform(dimension);
		} while (mBlocks[iRandom][jRandom] == 0);
		// then uniformly random select any allowed movement (except exchanging it with the blank tile)
		int iNew, jNew;
		boolean allowedMovement = false;
		do {
			int movement = StdRandom.uniform(4);
			switch (movement) {
			case 0: { // right
				iNew = iRandom;
				jNew = jRandom + 1;
			...
			if (iNew >=0 && iNew < dimension && jNew >= 0 && jNew < dimension)
				if (mBlocks[iNew][jNew] != 0) // we don't want to exchange with the blank tile 
					allowedMovement = true;
		} while (!allowedMovement);
		// swap positions once we found a valid movement
		int aux = mBlocks[iRandom][jRandom];
		mBlocks[iRandom][jRandom] = mBlocks[iNew][jNew];
		mBlocks[iNew][jNew] = aux;
		Board twinBoard = new Board(mBlocks);
		// revert back
		mBlocks[iNew][jNew] = mBlocks[iRandom][jRandom];
		mBlocks[iRandom][jRandom] = aux;
		return twinBoard;
	}
	...
	public Iterable<Board> neighbors() {
		...
		if (jBlank > 0) { // then we can swap it to its left neighbor (and then return the tile back to its original place)
			mBlocks[iBlank][jBlank] = mBlocks[iBlank][jBlank-1];
			mBlocks[iBlank][jBlank-1] = 0;
			neighbors.enqueue(new Board(mBlocks));
			mBlocks[iBlank][jBlank-1] = mBlocks[iBlank][jBlank];
			mBlocks[iBlank][jBlank] = 0;
		}
		...
		if (iBlank > 0) { // then we can swap it to its upper neighbor (and then return the tile back to its original place)
			mBlocks[iBlank][jBlank] = mBlocks[iBlank-1][jBlank];
			mBlocks[iBlank-1][jBlank] = 0;
			neighbors.enqueue(new Board(mBlocks));
			mBlocks[iBlank-1][jBlank] = mBlocks[iBlank][jBlank];
			mBlocks[iBlank][jBlank] = 0;
		}
	...
	public static void main(String[] args) {
		...
		System.out.println("isGoal should be true: [" + (board.isGoal() == true ? "OK" : "FAIL - " + board.isGoal()) + "]");
		...
		System.out.println("isGoal should be false: [" + (board2.isGoal() == false ? "OK" : "FAIL - " + board2.isGoal()) + "]");
		System.out.println("Printing neighbors:");
		nCount = 0;
		...
		System.out.println("isGoal should be true: [" + (board3.isGoal() == true ? "OK" : "FAIL - " + board3.isGoal()) + "]");
		System.out.println("Printing neighbors:");
		nCount = 0;
		...
		System.out.println("isGoal should be false: [" + (board4.isGoal() == false ? "OK" : "FAIL - " + board4.isGoal()) + "]");
		System.out.println("Printing neighbors:");
		nCount = 0;
		...
		Board board5 = new Board(new int[][]{{1,2,3},{4,5,0},{6,7,8}});
		...
		System.out.println("isGoal should be false: [" + (board5.isGoal() == false ? "OK" : "FAIL - " + board5.isGoal()) + "]");
		System.out.println("Printing neighbors:");
		nCount = 0;
		...
		System.out.println("====================\nTesting equals():");
		System.out.println("Board 1 equals to Board 1? Should be true: [" + (board.equals(board) == true ? "OK" : "FAIL - " + board.equals(board)) + "]");
		System.out.println("Board 1 equals to Board 2? Should be false: [" + (board.equals(board2) == false ? "OK" : "FAIL - " + board.equals(board2)) + "]");
		System.out.println("Board 1 equals to Board 3? Should be false: [" + (board.equals(board3) == false ? "OK" : "FAIL - " + board.equals(board3)) + "]");
		System.out.println("Board 3 equals to Board 3? Should be true: [" + (board3.equals(board3) == true ? "OK" : "FAIL - " + board3.equals(board3)) + "]");
